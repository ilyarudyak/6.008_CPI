...
def compute_posterior(prior, likelihood, y):
    ...
    for i in range(M):
        log_posterior[i] = log_prior[i]
        for yi in y:
            log_posterior[i] += log_likelihood[yi, i]
    
    norm = scipy.misc.logsumexp(log_posterior)
    log_posterior = log_posterior - norm
...
def infer_true_movie_ratings(num_observations=-1):
    ...
    for movie_id in range(num_movies):
        y = movie_data_helper.get_ratings(movie_id)
        posterior = compute_posterior(prior, likelihood, y)
        posteriors[movie_id] = posterior
        MAP_ratings[movie_id] = np.argmax(posterior)
...
def main():
    ...
    count = 0
    for movie_id in range(len(MAP_ratings)):
